...
class SavedChart(CompleteModel):  # Hereda de CompleteModel para tener auditoría y borrado lógico
    ...
    def to_json(self):
        """Retorna una representación JSON del gráfico para su uso en el frontend"""
        return {
            'id': self.id,
            'title': self.title,
            'description': self.description,
            'chart_type': self.chart_type_id,
            'model_content_type': self.model_content_type_id,
    ...
    def clean(self):
        ...
        if not model_class:
            raise ValidationError(_("Modelo inválido"))
            
        self.validate_field_exists(model_class, self.x_axis_field, "X")
        
        if self.y_axis_field != "count":
            self.validate_field_exists(model_class, self.y_axis_field, "Y")

    def validate_field_exists(self, model, field, axis):
        if not hasattr(model, field):
            raise ValidationError(
                _("Campo %(axis)s inválido: %(field)s no existe"),
                params={"axis": axis, "field": field}
            )
